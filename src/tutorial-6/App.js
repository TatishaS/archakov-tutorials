...
export default function App() {
  ...
  const articleNumber = pathname.split('/').pop();
  ...
  /*   const articles = [
    ...
    {
      id: 2,
      text: `Using color to add meaning only provides a visual indication, which will not be conveyed to users of assistive technologies – such as screen readers. Ensure that information denoted by the color is either obvious from the content itself (e.g. the visible text)`,
    },
  ];
  ...
  return (
    <div className="App">
      ...
      {/* ТУТ БЫЛ ВАРИАНТ С РЕГУЛЯРНЫМИ ВЫРАЖЕНИЯМИ ДЛЯ ЛЮБОЙ СТРОКИ ПОСЛЕ СЛЕША */}

      {pathname === `/post/${articleNumber}` && <Article id={articleNumber} />}
